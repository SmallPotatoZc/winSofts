...
var emlformat = {
  ...
  unquoteUTF8: function(s) {
    ...
    if (match) {
      ...
      var value = match[2];
      if (type == "B") { //Base64
        console.log('----1', value.replace(/\r?\n/g, ""));
        window.atob(value.replace(/\r?\n/g, ""));
        // return Buffer.from(value.replace(/\r?\n/g, ""), "base64").toString("utf8");
        return window.btoa(value.replace(/\r?\n/g, ""));
      }
  ...
  unquotePrintable: function(s) {
    ...
    return s
    .replace(/=([\w\d]{2})=([\w\d]{2})=([\w\d]{2})/gi, function(matcher, p1, p2, p3, offset, string) { return new ArrayBuffer([ parseInt(p1, 16), parseInt(p2, 16), parseInt(p3, 16) ]).toString("utf8"); })
    .replace(/=([\w\d]{2})=([\w\d]{2})/gi, function(matcher, p1, p2, offset, string) { return new ArrayBuffer([ parseInt(p1, 16), parseInt(p2, 16) ]).toString("utf8"); })
    ...
    if (data.attachments) {
      for (var i = 0; i < data.attachments.length; i++) {
        ...
        if (typeof attachment.data == "string") {
          console.log('----2', attachment.data);
          // var content = Buffer.from(attachment.data).toString("base64");
          var content = window.atob(attachment.data);
...
emlformat.read = function(eml, options, callback) {
  ...
  if (typeof eml == "string") {
    ...
    callback("Missing EML file content!");
  }
};
